...
public class Transaction {
    ...
    public boolean processTransaction() {
        ...
        for(TransactionInput i : inputs) {
            i.UTXO =NVT_BlockChain.UTXOs.get(i.transactionOutputId);
        }

        //Kiểm tra giao dịch hợp lệ
        if(getInputsValue() < NVT_BlockChain.minimumTransaction) {
            System.out.println("Đầu vào của giao dịch quá bé: " + getInputsValue());
            System.out.println("Vui lòng nhập số điện thoại lớn hơn " + NVT_BlockChain.minimumTransaction);
            return false;
        }
        ...
        for(TransactionOutput o : outputs) {
            NVT_BlockChain.UTXOs.put(o.id , o);
        }
        ...
        for(TransactionInput i : inputs) {
            ...
            NVT_BlockChain.UTXOs.remove(i.UTXO.id);
        }
